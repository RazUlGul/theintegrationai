...
export const VanishInput = ({
  ...
  return (
    <div className={`relative py-8 min-h-[380px] lg:min-h-[640px]`}>
      <div ref={scrollContainerRef} className="mt-6 max-h-[420px] lg:max-h-[680px] overflow-y-scroll space-y-3 px-4 custom-scroll">
      ...
      <div className="flex justify-center w-full relative mt-8 px-4">
        <input
          ...
          required={required}
          type={type}
          placeholder={(!isFocused && conversation.length) === 0 ? placeholder : ""}
        ...
        />
        {!isFocused && (
          <div className="absolute top-1/2 translate-y-[-50%] left-[calc(50%-140px)]">
            <span className="inline-block w-[2px] h-[24px] bg-white animate-blink"></span>
          </div>
        )}
      </div>
      <div className={`flex justify-center items-center absolute ${active ? 'bottom-[-100px]' : 'bottom-20'} left-1/2 -translate-x-1/2 gap-4 py-8 hover:scale-105 cursor-pointer duration-300 transition-all`} onClick={sendMessage}>
        <div className="bg-gray-500 leading-normal text-[18px] w-[32px] h-[32px] relative rounded-md">
